...
export default function PortalsTab({ theme = 'glassNavy', initialSubTab = 'home', lang = 'en' }: PortalsTabProps) {
  ...
  if (initialSubTab === 'home') {
    return (
      <div className={`rounded-3xl overflow-hidden ${isGlass ? 'text-[#431407]' : 'text-white'}`}>
        {/* Hero Section */}
        <div className={`relative px-8 py-24 flex flex-col items-center justify-center text-center overflow-hidden ${isGlass ? 'bg-gradient-to-br from-[#FFEDD5] via-[#FEEBD8] to-[#FED7AA]' : 'bg-gradient-to-br from-[#022c22] via-[#064e3b] to-[#0f172a]'}`}>
          ...
          <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
            ...
            <div className="absolute bottom-[-10%] right-[-5%] w-[30rem] h-[30rem] rounded-full blur-[120px] opacity-30 animate-pulse-glow bg-[#3B82F6]" style={{ animationDelay: '1.5s' }}></div>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h1 className="text-5xl md:text-7xl font-black mb-6 tracking-tight animate-slideUp">
              <span className={`text-transparent bg-clip-text bg-gradient-to-r ${isGlass ? 'from-[#431407] via-[#F97316] to-[#431407]' : 'from-white via-[#EA580C] to-white'} animate-shimmer`} style={{ backgroundSize: '200% auto' }}>
            ...
            <p className="text-lg md:text-2xl font-medium max-w-3xl opacity-80 mb-10 animate-slideUp" style={{ animationDelay: '0.1s' }}>
              Empowering minds, building character, and shaping the future leaders of tomorrow through holistic education and innovation.
            </p>
            
            <div className="flex flex-col sm:flex-row gap-5 animate-slideUp" style={{ animationDelay: '0.2s' }}>
              <button 
                onClick={() => {
                  const studentName = prompt("Enter Student Full Name for Admission:");
                  if (!studentName) return;
                  const fatherName = prompt("Enter Father's Name:");
                  if (!fatherName) return;
                  const classTarget = prompt("Enter Target Class (e.g. Class I):");
                  ...
                  alert("Application submitted successfully! Our administration will review it in the Admin Dashboard.");
                }}
                className={`px-8 py-4 rounded-full text-lg font-bold flex items-center gap-3 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${isGlass ? 'bg-gradient-to-r from-[#F97316] to-orange-400 text-white shadow-orange-400/30' : 'bg-gradient-to-r from-[#EA580C] to-rose-500 text-black shadow-rose-500/30'}`}
              >
                Apply for Admission <ArrowRight className="w-5 h-5 animate-pulse" />
              </button>
              <button 
                onClick={() => {
                  const name = prompt("Enter Student Name for Campus Tour Booking:");
                  if (!name) return;
                  const date = prompt("Enter preferred Date (e.g. YYYY-MM-DD):");
                  if (!date) return;
                  ...
                  alert("Campus Tour booked successfully! Our administration will confirm shortly.");
                }}
              ...
              >
                <Video className="w-5 h-5" /> Campus Tour
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-6 sm:px-8 -mt-8 relative z-20">
          {[
            { label: 'Pass Rate', value: '100%', icon: Award, color: 'text-amber-500' },
            { label: 'Students', value: '2,500+', icon: Users, color: 'text-blue-500' },
            { label: 'Courses', value: '50+', icon: BookOpen, color: 'text-purple-500' },
            { label: 'Est.', value: '1995', icon: CheckCircle2, color: 'text-rose-500' },
          ].map((stat, i) => (
        ...
        <div className="px-6 sm:px-8 mt-16 animate-slideUp">
          <div className={`p-8 sm:p-10 rounded-3xl border ${isGlass ? 'bg-white/60 border-white/40 backdrop-blur-xl' : 'bg-[#1C1C1F]/60 border-white/10 backdrop-blur-xl'} shadow-xl`}>
            <div className="flex items-center justify-center gap-3 mb-10">
              ...
              <h2 className="text-3xl md:text-4xl font-black text-center">Hall of Fame</h2>
              <Star className="w-8 h-8 text-amber-500 fill-amber-500 animate-pulse-glow" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-6 lg:gap-10">
              ...
              <div className="flex flex-col items-center justify-center text-center group">
                <div className={`relative w-40 h-40 sm:w-48 sm:h-48 rounded-full p-2 mb-6 transition-transform duration-500 group-hover:scale-105 ${isGlass ? 'bg-gradient-to-tr from-rose-400 to-orange-400' : 'bg-gradient-to-tr from-rose-500 to-orange-500'} shadow-lg`}>
              ...
              <div className="flex flex-col items-center justify-center text-center group md:-mt-8">
                <div className={`relative w-48 h-48 sm:w-56 sm:h-56 rounded-full p-2 mb-6 transition-transform duration-500 group-hover:scale-105 ${isGlass ? 'bg-gradient-to-tr from-amber-300 to-yellow-500' : 'bg-gradient-to-tr from-amber-400 to-yellow-600'} shadow-2xl`}>
                ...
                <p className="text-sm opacity-80 font-bold mb-2">Class IX • National Science Olympiad Gold</p>
                <p className="text-xs opacity-60 font-medium max-w-xs mx-auto">Consistently demonstrating exceptional intellect, humility, and leadership at Saint Xavier's.</p>
              </div>

              {/* Star of the Month */}
              <div className="flex flex-col items-center justify-center text-center group">
                <div className={`relative w-40 h-40 sm:w-48 sm:h-48 rounded-full p-2 mb-6 transition-transform duration-500 group-hover:scale-105 ${isGlass ? 'bg-gradient-to-tr from-teal-400 to-emerald-400' : 'bg-gradient-to-tr from-orange-400 to-rose-500'} shadow-lg`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 sm:p-8 mt-4">
          ...
          <div className={`group p-8 rounded-3xl border transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 ${isGlass ? 'bg-white/70 border-white/50 backdrop-blur-lg hover:bg-white/90' : 'bg-[#1C1C1F]/80 border-white/10 backdrop-blur-lg hover:bg-[#2C2C2E]/90'}`}>
            ...
            <h3 className="text-2xl font-black mb-4">Latest News</h3>
            <ul className="space-y-4 opacity-80 font-medium">
              <li className="flex items-start gap-3 group/item">
                ...
                <span className="group-hover/item:text-amber-500 transition-colors">Annual Sports Meet 2026 scheduled for November.</span>
              </li>
              <li className="flex items-start gap-3 group/item">
                <span className="text-amber-500 mt-1 transition-transform group-hover/item:translate-x-1">→</span> 
                <span className="group-hover/item:text-amber-500 transition-colors">Board Examination dates announced by CBSE.</span>
              </li>
              <li className="flex items-start gap-3 group/item">
                <span className="text-amber-500 mt-1 transition-transform group-hover/item:translate-x-1">→</span> 
                <span className="group-hover/item:text-amber-500 transition-colors">Winter vacation starting from 25th Dec.</span>
              </li>
            </ul>
          </div>
          ...
          <div className={`group p-8 rounded-3xl border transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 ${isGlass ? 'bg-white/70 border-white/50 backdrop-blur-lg hover:bg-white/90' : 'bg-[#1C1C1F]/80 border-white/10 backdrop-blur-lg hover:bg-[#2C2C2E]/90'}`}>
            ...
            <ul className="space-y-4 opacity-80 font-medium">
              <li className="flex items-start gap-3 group/item">
                ...
                <span className="group-hover/item:text-purple-500 transition-colors">100% Pass result in Class XII Board Exams.</span>
              </li>
              <li className="flex items-start gap-3 group/item">
                <span className="text-purple-500 mt-1 transition-transform group-hover/item:translate-x-1">★</span> 
                <span className="group-hover/item:text-purple-500 transition-colors">Won Inter-School Debate Championship.</span>
              </li>
              <li className="flex items-start gap-3 group/item">
                <span className="text-purple-500 mt-1 transition-transform group-hover/item:translate-x-1">★</span> 
                <span className="group-hover/item:text-purple-500 transition-colors">Best Eco-Friendly Campus Award 2025.</span>
              </li>
            </ul>
          </div>
  ...
  if (initialSubTab === 'parent_login' || initialSubTab === 'teacher_login' || initialSubTab === 'student_login') {
    ...
    return (
      <div className={`flex flex-col items-center justify-center min-h-[600px] ${isGlass ? 'text-[#431407]' : 'text-white'}`}>
        <div className={`w-full max-w-md p-8 rounded-3xl border shadow-xl ${isGlass ? 'bg-white/90 backdrop-blur-xl border-white/40' : 'bg-[#1C1C1F] border-[#2C2C2E]'}`}>
          <div className="flex flex-col items-center text-center mb-6">
            ...
            <p className="text-sm opacity-60">Authorize workspace to access your dynamic {role.title.toLowerCase()}.</p>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleLoginFormSubmit}>
            <div>
              <label className="block text-xs font-bold mb-2 opacity-80 uppercase tracking-wider">Username or ID</label>
            ...
            <div>
              <label className="block text-xs font-bold mb-2 opacity-80 uppercase tracking-wider">Password</label>
              <div className="relative">
            ...
            <div className="flex items-center justify-between text-xs font-bold">
              ...
              <span className={`hover:underline cursor-pointer ${role.color}`}>Forgot Password?</span>
            </div>
            <button className={`w-full py-3.5 rounded-xl font-black text-white transition hover:opacity-90 flex items-center justify-center gap-2 ${role.title === 'Parent Portal' ? 'bg-orange-400' : role.title === 'Teacher Portal' ? 'bg-indigo-500' : 'bg-amber-500'}`}>
              <Key className="w-4 h-4" /> Secure Login
            </button>
          </form>
    ...
    return (
      <div className={`p-6 sm:p-8 rounded-3xl min-h-[600px] border ${isGlass ? 'bg-white/80 backdrop-blur-xl border-white/40 text-[#431407]' : 'bg-[#1C1C1F] border-[#2C2C2E] text-white'}`}>
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-black flex items-center gap-2"><BookOpen className="text-rose-500" /> Digital Homework</h2>
            <p className="opacity-60 text-sm mt-1">Recent assignments and submissions</p>
          </div>
          <button className={`px-4 py-2 rounded-lg text-sm font-bold ${isGlass ? 'bg-[#431407] text-white shadow-sm' : 'bg-white text-black'}`}>Upload Assignment</button>
        </div>
            ...
            <div key={i} className={`p-4 rounded-xl border flex items-center justify-between transition hover:scale-[1.01] ${isGlass ? 'bg-white border-gray-100 shadow-sm' : 'bg-[#0F0F12] border-[#2C2C2E]'}`}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs opacity-60 flex items-center gap-1 mt-1"><Clock className="w-3 h-3" /> Due: {hw.due}</p>
                </div>
              </div>
  ...
  if (initialSubTab === 'live_class') {
    return (
      <div className={`p-6 sm:p-8 rounded-3xl min-h-[600px] border flex flex-col ${isGlass ? 'bg-white/80 backdrop-blur-xl border-white/40 text-[#431407]' : 'bg-[#1C1C1F] border-[#2C2C2E] text-white'}`}>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-black flex items-center gap-2"><Video className="text-red-500" /> Live Virtual Classrooms</h2>
  ...
  if (initialSubTab === 'classroom_cameras') {
    return (
      <div className={`p-6 sm:p-8 rounded-3xl min-h-[600px] border ${isGlass ? 'bg-white/80 backdrop-blur-xl border-white/40 text-[#431407]' : 'bg-[#1C1C1F] border-[#2C2C2E] text-white'}`}>
        <h2 className="text-2xl font-black flex items-center gap-2 mb-6"><Camera className="text-purple-500" /> AI Classroom Surveillance</h2>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className={`text-2xl font-black flex items-center gap-2 ${isPresent ? 'text-orange-400' : 'text-red-500'}`}>
              ...
              {isPresent ? 'Today Present Students' : 'Today Absent Students'}
            </h2>
            <p className="opacity-60 text-sm mt-1">Date: {new Date().toLocaleDateString()}</p>
          </div>
  ...
  if (initialSubTab === 'scholarship') {
    return (
      <div className={`p-6 sm:p-8 rounded-3xl min-h-[600px] border ${isGlass ? 'bg-white/80 backdrop-blur-xl border-white/40 text-[#431407]' : 'bg-[#1C1C1F] border-[#2C2C2E] text-white'}`}>
        <div className={`p-8 rounded-2xl mb-8 flex flex-col md:flex-row items-center justify-between gap-6 ${isGlass ? 'bg-gradient-to-r from-amber-100 to-orange-100 shadow-sm' : 'bg-gradient-to-r from-amber-500/20 to-orange-600/20'}`}>
          <div>
            <span className="px-3 py-1 bg-amber-500 text-white rounded-full text-xs font-bold mb-3 inline-block">OPEN NOW</span>
            <h2 className="text-3xl font-black mb-2">Merit Scholarship 2026</h2>
            <p className="opacity-80 max-w-lg">Up to 100% tuition fee waiver for outstanding academic performers. Apply before 31st August.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="text" placeholder="Student Full Name" className={`p-3 rounded-xl border ${isGlass ? 'bg-white border-gray-200' : 'bg-[#0F0F12] border-[#2C2C2E]'}`} />
          <input type="text" placeholder="Enrollment / Scholar No" className={`p-3 rounded-xl border ${isGlass ? 'bg-white border-gray-200' : 'bg-[#0F0F12] border-[#2C2C2E]'}`} />
          <input type="text" placeholder="Current Class & Section" className={`p-3 rounded-xl border ${isGlass ? 'bg-white border-gray-200' : 'bg-[#0F0F12] border-[#2C2C2E]'}`} />
          <input type="text" placeholder="Previous Year % or Grade" className={`p-3 rounded-xl border ${isGlass ? 'bg-white border-gray-200' : 'bg-[#0F0F12] border-[#2C2C2E]'}`} />
          <div className="md:col-span-2">
            <textarea placeholder="Why do you deserve this scholarship?" rows={4} className={`w-full p-3 rounded-xl border ${isGlass ? 'bg-white border-gray-200' : 'bg-[#0F0F12] border-[#2C2C2E]'}`}></textarea>
          </div>
          <button className="md:col-span-2 p-3 bg-[#F97316] hover:bg-[#2C7A7B] text-white font-bold rounded-xl transition">Submit Application</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`p-6 rounded-2xl ${isGlass ? 'bg-white/70 backdrop-blur-xl text-[#431407]' : 'bg-[#0F0F12] text-white'}`}>
      <h2 className="text-xl font-bold mb-4">Module Under Construction</h2>
      <p>This section ({initialSubTab}) is currently being developed.</p>
    </div>
  );
}
